...
import numpy as np
from MaP_evaluation.scripts.extra.intersect_gt_and_dr import *

def MAP_eval(MINOVERLAP, writer, pred_flag):
    ...
    if(pred_flag):
        ...
        DR_PATH = '../results/Output'
    else:
        GT_PATH = '../results/GroundTruth'
        DR_PATH = '../results/Input'
    # if there are no images then no animation can be shown
    intersect_gt_dr(GT_PATH, DR_PATH)
    ...
    def draw_plot_func(dictionary, n_classes, window_title, plot_title, x_label, output_path, to_show, plot_color, true_p_bar):
        ...
        plot_to_tensorboard(writer, plot_title, fig, 0)
        # show image
        if to_show:
            plt.show()
        # close the plot
    ...
    with open(output_files_path + "/output.txt", 'w') as output_file:
        ...
        for class_index, class_name in enumerate(gt_classes):
            ...
            if draw_plot:
                ...
                # if(pred_flag):
                #     plot_to_tensorboard(writer, class_name+'-predicted', fig, 0)
                # else:
                #     plot_to_tensorboard(writer, class_name + '-noisy', fig, 0)
    ...
    if draw_plot:
        window_title = "lamr"
        if pred_flag:
            plot_title = "log-average miss rate - Predicted"
        else:
            plot_title = "log-average miss rate - Noisy"
    ...
    if draw_plot:
        window_title = "mAP"
        if pred_flag:
            plot_title = "Predicted mAP Minoverlap = {0:.1f}%".format(MINOVERLAP)
        else:
            plot_title = "Noisy mAP Minoverlap = {0:.1f}%".format(MINOVERLAP)
        x_label = "Average Precision = {0:.2f}%".format(mAP * 100)
